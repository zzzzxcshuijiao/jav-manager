...
M1 当前已通过的 focused 验证：

- `rustfmt --check src-tauri/src/inventory_move.rs src-tauri/src/inventory_execution.rs src-tauri/tests/inventory_execution.rs`
- `cargo test --manifest-path src-tauri/Cargo.toml --test inventory_execution -j 1`（17 tests）
- `cargo test --manifest-path src-tauri/Cargo.toml inventory_move::tests:: -j 1`（6 tests）
- `npm test -- src/App.inventory.test.tsx`（6 tests）
- `npm test -- src/viewModel.test.ts`（50 tests）
- `npx tsc --noEmit`
...
cd media-manager
git checkout codex/m1-inventory-centralized-migration
...
38. **`docs/superpowers/plans/2026-06-28-media-manager-m1-centralized-inventory-migration.md`** — M1 存量资源集中迁移实施计划（当前主线，已实施到后端迁移执行与前端主入口）。
...
- `InventoryExecutionMode` 新增 `move`，保留 `copy` / `low_space` 兼容旧调用。
- `InventoryExecutionActionStatus` 新增 `moved` / `rollback_failed`；执行报告新增 `moved_actions`、`bytes_moved`、`same_volume_actions`、`cross_volume_actions`、`space_blocked_actions`、`rollback_failed_actions`。
- 新增 `src-tauri/src/inventory_move.rs`：同卷 hard link + 校验 + 隔离删除源，跨卷 copy-verify-delete，空间检查，no-clobber 发布，源路径隔离后校验并删除，避免 TOCTOU 按路径误删。
- 同卷误判但 hard link 返回 cross-device 时，会 fallback 到跨卷 copy-verify-delete。
- 执行器只消费 `resolution.execution_plan.actions`；失败回滚边界限定在当前作品，已完成前序作品继续计入 `executed_works`。
...
当前主线是 **M1：存量资源集中迁移**。后端迁移执行、前端主入口和完整 gate 已经完成，下一步是 push 分支，然后在真实环境用小样本做集中迁移验证。不要再把默认执行路线扩展回 7D 的硬链接整理；`copy` / `low_space` 只作为兼容和保守诊断能力保留。
...
3. 先准备 1-3 个可接受迁移的小样本作品目录，避免第一次直接对全量真实库执行。
4. 在“一键盘点”里每行填一个样本根目录，例如视频目录、NFO 输出目录、图片目录。
5. 填写“整理目标目录”，建议先用空目录或测试目标库目录。
6. 点击“开始盘点”；期望只生成汇总、复核分桶、作品列表、资源证据、配对建议、安全执行计划、候选动作预览、warnings 和目标路径预览，不移动、不复制、不删除任何文件。
7. 重点看“可自动整理 / 需人工确认 / 阻断 / 素材候选 / 孤儿资源”筛选是否符合真实目录情况，并打开作品详情检查主视频、主 NFO、资源角色、安全执行计划、候选动作重复和阻断原因。
8. 如果 `可自动整理` 大于 0 且报告未截断，可点击“集中迁移”；确认框会提示成功后源路径不再保留、同盘直接移动、跨盘复制校验后删除源文件、目标已存在不会覆盖。
9. 迁移完成后检查：目标目录出现 `archive_root/CODE/` 布局；已迁移源文件消失；NFO/图片/GIF 等关联资源一起进入目标目录；最近集中迁移摘要显示 `moved` / 同盘 / 跨盘 / 空间不足等统计。
10. 整理后重新点击“开始盘点”验证目标状态；如果重复执行，目标已存在应被识别为冲突并阻止覆盖。
11. 点击“导出 JSON”，导出当前 report 到 app data 的 `inventory-reports/`；如果需要后续分析，把导出的 JSON 路径或文件发给 Codex。

如果继续做 Codex 可编码工作，优先 push 当前分支并交付真实环境测试说明。迁移结果入库同步、目标库重建、人工复核项执行、孤儿资源整理、旧迁移入口清理、typed execution errors 和空目录清理放到 M1 之后。仍然不要在 Codex 会话里启动 Tauri GUI 或 WebView2；视觉检查由用户在自己的交互桌面环境运行。
